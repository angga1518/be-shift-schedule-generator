...
def _add_non_shift_limit_constraints(solver: ScheduleSolver):
    """
    Limits the maximum number of days non-shift personnel can work in total.
    ...
    # For each non-shift personnel, count total working days across the month
    for p_id in non_shift_personnel:
        ...
        solver.model.Add(sum(working_days) <= solver.config.max_non_shift)
